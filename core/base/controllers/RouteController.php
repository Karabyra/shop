    ...
    private function createRoute($var, $arr)
    {
        ...
        if (!empty($arr[0])) {
            if ($this->routes['var']['routes'][$arr[0]]) {
                ...
                $this->controller .= unfirst($arr[0] . 'Controller');
            }

        } else {
            $this->controller .= $this->routes['default']['controller'];
        }
        $this->inputMethod = $route[1] ? $route : $this->routes['default']['inputMethod'];
        $this->outputMethod = $route[2] ? $route : $this->routes['default']['outputMethod'];
